...
public class Obliterator
{
    ...
    public static void Obliterate(String imgName, String ResultName, int planeToRemove)
    {
        BufferedImage image = getImage(imgName);

        image = removePlaneAndBellow(image, planeToRemove);

        setImage(image, new File(ResultName), "png");
    }

    public static BufferedImage removePlaneAndBellow(BufferedImage image, int planeToRemove)
    {
        byte img[] = get_byte_data(image);
        try
        {
            removePlaneAndBellowByte(img, planeToRemove);
        } catch (Exception e)
        {
    ...
    private static byte[] removePlaneAndBellowByte(byte[] image, int planeToRemove)
    {
        ...
        for (int i = 0; i < image.length / 8; ++i)
        {
            ...
            for (int bit = 7; bit >= 0; --bit, ++offset) //ensure the new offset value carries on through both loopsobl
            {
                ...
                //image[offset] = (byte) (image[offset] + 10);

                image[offset] = (byte) (image[offset] & ((int) (0xFF - (Math.pow(0x02, (planeToRemove))))) | b);
            }
        }
